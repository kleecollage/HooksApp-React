...
export type ScrambleWordsActions =
  ...
  | { type: 'CHECK_ANSWER' }
  | { type: 'SKIP_WORD', payload: number }
  | { type: 'START_NEW_GAME', payload: ScrambleWordState }


const GAME_WORDS = [
  'REACT',     'JAVASCRIPT',   'TYPESCRIPT',   'HTML',         'ANGULAR',
  'SOLID',     'NODE',         'VUEJS',        'SVELTE',       'EXPRESS',
  'MONGODB',   'POSTGRES',     'DOCKER',       'KUBERNETES',   'WEBPACK',
  'VITE',      'TAILWIND',
];
...
export const scrambleWordsReduce = (state: ScrambleWordState, action: ScrambleWordsActions): ScrambleWordState => {
  switch (action.type) {
    case 'SET_GUESS':
      return {
        ...state,
